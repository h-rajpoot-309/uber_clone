...
import 'package:uber_clone/common/controller/services/APIsNKeys/apis.dart';
...
class LocationServices {
  ...
  static Future getAddressFromLatLng({
    ...
    // final apiUrl = APIs.geoCodingAPI(position);
    // print('Geocoding URL: $apiUrl');
    //
    ...
    try {
      ...
      if (response.statusCode != 200) {
        // Show toast or log if you want
        ToastService.sendScaffoldAlert(
          msg: 'Failed to fetch address (${response.statusCode})',
          toastStatus: 'ERROR',
          context: context,
        );
      ...
      if (decodedResponse['results'] == null ||
          decodedResponse['results'].isEmpty) {
        throw Exception('No address results from API');
      }
